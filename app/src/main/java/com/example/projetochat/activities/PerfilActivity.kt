...
import com.example.projetochat.databinding.ActivityPerfilBinding
...
class PerfilActivity : AppCompatActivity() {
    ...
    private fun solicitarPermissoes() {
        //Verifica se usuario já tem permissão
        ...
        val listaPermissoesNegada = mutableListOf<String>()
        if ( !temPermissaoCamera )
            listaPermissoesNegada.add( Manifest.permission.CAMERA )
        if ( !temPermissaoGaleria )
            listaPermissoesNegada.add( Manifest.permission.READ_EXTERNAL_STORAGE )

        if ( listaPermissoesNegada.isNotEmpty() ){
            ...
            gerenciadorPermissoes.launch( listaPermissoesNegada.toTypedArray() )
        }
